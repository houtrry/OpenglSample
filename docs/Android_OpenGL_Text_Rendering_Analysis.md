...
> 备注（与项目 Hybrid 策略对齐）：对于复杂脚本/Emoji/合字等 SDF 不易正确渲染或命中率低的文本，建议直接回退 Canvas/Complex 路径，保障多语言正确渲染的一致性，再由混合策略综合权衡性能与观感。
...
> 备注：混合方案中，复杂脚本（如阿语形态连接、双向文本、Emoji 合字等）优先回退 Canvas/Complex；拉丁/CJK 等通用字符优先走 SDF，确保在 2000+ 规模下的批量渲染效率与正确性兼顾。
