...
data class BubbleTextLayoutParam(
     ...
     * 背景色
     */
    val backgroundColor: Int = Color.argb(255, 234, 234, 234),
...
fun BubbleText.generateBitmap(isLTR: Boolean = isLTR()): Bitmap {
    ...
    drawDrawable(canvas, paint, bubbleSize, offsetX, isLTR)
    drawText(canvas, paint, bubbleSize, offsetX, isLTR)
    return bitmap
}
...
fun BubbleText.drawText(canvas: Canvas, paint: Paint, size: Size, offsetX: Int, isLTR: Boolean) {
    val left: Float
    val bottom: Float
    val adjustText = if (isLTR) text else text.reversed()
    paint.textSize = textParams.textSize
    paint.color = textParams.textColor
    paint.strokeWidth = textParams.textStokeWidthSize
    paint.style = Paint.Style.FILL
    val textWidth = paint.measureText(adjustText)
...
fun BubbleText.update(vector3: Vector3 ?= null, text: String? = null, color: Int? = null): BubbleText {
    return when{
        text == null && color == null -> this
        text != null && color != null -> this.copy(text = text, textParams = this.textParams.copy(textColor = color))
        text != null -> this.copy(text = text)
        color != null -> this.copy(textParams = this.textParams.copy(textColor = color))
        else -> this
    }
}